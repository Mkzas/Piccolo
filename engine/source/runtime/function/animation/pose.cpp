...
void AnimationPose::blend(const AnimationPose& pose)
{
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
            ...
            float next_weight = pose.m_weight.m_blend_weight[i];
            m_weight.m_blend_weight[i] = (next_weight - cur_weight) / sum_weight;

            bone_trans_one.m_position = 
                cur_weight / sum_weight * bone_trans_one.m_position +
                                        next_weight / sum_weight * bone_trans_two.m_position;

            bone_trans_one.m_scale =
                cur_weight / sum_weight * bone_trans_one.m_scale + next_weight / sum_weight * bone_trans_two.m_scale;

            bone_trans_one.m_rotation =
                Quaternion::nLerp(next_weight / sum_weight, bone_trans_one.m_rotation, bone_trans_two.m_rotation, true);
        }
    }
}
